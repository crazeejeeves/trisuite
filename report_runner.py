...
def case_name(name):
    ...
    def decorator(function):
        function.func_dict['test_case_name'] = name
        return function
    return decorator
...
def skip_device(name):
    ...
    def decorator(function):
        ...
        function.func_dict['skip_device'] = name_list
        return function
    return decorator
...
def _set_test_type(function, test_type):
    """ Test type setter
    """
    if 'test_type' in function.func_dict:
        function.func_dict['test_type'].append(test_type)
    else:
        function.func_dict['test_type'] = [test_type]
    return function
...
class CustomTextTestResult(result.TestResult):
        ...
        if os.path.isfile(self.results_file_path):
            with open(self.results_file_path, 'rb') as f:
                try:
                    ...
                    self.results['suites'] = {int(k):v for (k,v) in self.results['suites'].iteritems()}
                    self.suite_map = {v['name']:int(k) for (k,v) in self.results['suites'].iteritems()}
                    self.previous_suite_runs = self.results['suites'].keys()
                except:
                    pass
        if not self.results:
            self.results = {'suites': {},
    ...
    def stopTestRun(self):
        ...
        if not os.path.exists(self.results_file_path):
            dir = os.path.dirname(self.results_file_path)
    ...
    def startTest(self, test):
        ...
        try:
            name_override = getattr(test, test._testMethodName).__func__.func_dict['test_case_name']
        except:
            name_override = None
        ...
        if self.test_types:
            if ('test_type' in getattr(test, test._testMethodName).__func__.func_dict
                and set([s.lower() for s in self.test_types]) == set([s.lower() for s in getattr(test, test._testMethodName).__func__.func_dict['test_type']])):
                pass
            else:
                getattr(test, test._testMethodName).__func__.func_dict['__unittest_skip_why__'] = 'Test run specified to only run tests of type "%s"' %','.join(self.test_types)
                getattr(test, test._testMethodName).__func__.func_dict['__unittest_skip__'] = True
        if 'skip_device' in getattr(test, test._testMethodName).__func__.func_dict:
            for device in getattr(test, test._testMethodName).__func__.func_dict['skip_device']:
                if self.config and device.lower() in self.config['device_name'].lower():
                    getattr(test, test._testMethodName).__func__.func_dict['__unittest_skip_why__'] = 'Test is marked to be skipped on %s' %device
                    getattr(test, test._testMethodName).__func__.func_dict['__unittest_skip__'] = True
                    break
    ...
    def addSkip(self, test, reason):
        ...
        self.results['suites'][self.suite_map[self.suite]]['cases'][self.current_case_number]['note'] = getattr(test, test._testMethodName).__func__.func_dict['__unittest_skip_why__']
        self.num_skipped += 1
    ...
    def printErrorsOverview(self, r):
        ...
        data = []
        for suite in r:
            cases = {k:v for (k,v) in r[suite]['cases'].iteritems() if v['failures'] or v['errors']}
            map(lambda x: data.append({'suite': '%s%s' %(r[suite]['name'], ' (%s)' %r[suite]['module'] if r[suite]['class'] != r[suite]['name'] else ''),
                                       'case': '%s%s' %(cases[x]['name'], ' (%s)' %cases[x]['method'] if cases[x]['name'] != cases[x]['method'] else ''),
                                       'result': cases[x]['result'].upper()}), cases)
        self.stream.writeln( TablePrinter(fmt, tl=self.separator1, ul=self.separator2)(data) )
        self.stream.writeln(self.separator3)
        self.stream.writeln()
    ...
    def printErrorsDetail(self, r):
        ...
        for suite in r:
            failures_and_errors = [k for (k,v) in r[suite]['cases'].iteritems() if v['failures'] or v['errors']]
    ...
    def printSkippedDetail(self, r):
        ...
        data = []
        for suite in r:
            cases = {k:v for (k,v) in r[suite]['cases'].iteritems() if v['result'] == 'skipped'}
            map(lambda x: data.append({'suite': '%s%s' %(r[suite]['name'], ' (%s)' %r[suite]['module'] if r[suite]['class'] != r[suite]['name'] else ''),
                                       'case': '%s%s' %(cases[x]['name'], ' (%s)' %cases[x]['method'] if cases[x]['name'] != cases[x]['method'] else ''),
                                       'reason': cases[x]['note']}), cases)
        self.stream.writeln( TablePrinter(fmt, tl=self.separator1, ul=self.separator2)(data) )
        self.stream.writeln(self.separator3)
        self.stream.writeln()
